class Api::V1::StatusLogsController < ApplicationController
  before_action :set_api_v1_status_log, only: %i[show update destroy]

  # GET /api/v1/status_logs
  def index
    @api_v1_status_logs = StatusLog.all
  ...
  def create
    @api_v1_status_log = StatusLog.new(api_v1_status_log_params)
  ...
  def set_api_v1_status_log
    @api_v1_status_log = StatusLog.find(params[:id])
  end
